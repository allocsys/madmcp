...
- **New (2026-09-01): genuine QStash worker-chain stall observed live,
  ...
  **Correction (2026-09-01, later still): the live per-step-cadence
  check for this fix was confounded by the polling methodology itself
  -- within-poll cadence is actually flat, which is a positive signal,
  not a negative one.** While validating this fix and the still-open
  `.transient` fix together on a fresh `bai`-provider full
  `connectors/` audit (run_id
  `13dae425-5575-4fe8-9efb-5181af7ffb02`), repeated no-`max_steps`
  polling appeared to reproduce the exact ~10s -> 90s -> 3min widening
  gap the fix targeted, on `/api/agent-worker` logs pulled via
  `Vercel:get_runtime_logs`. `Vercel:get_deployment` confirmed the
  serving deployment (`dpl_2ZqKTweHDG6Vpzm3X9KG2iE9Sn1Y`) was in fact
  built from `githubCommitSha: 27449d587f9cbada9629ba347c28e0541c78a72e`
  -- the fix commit, not a stale pre-fix build -- so on first read this
  looked like the fix not working on the code that should have it.

  Re-examining the same log window *without* filtering to
  `/api/agent-worker` alone revealed the confound: interleaved
  `POST /mcp/...` entries -- the MCP server calls generated by this
  session's own `resume_run_id` polls -- line up almost exactly with
  the start of every fast, flat burst of `/api/agent-worker` calls
  (consistently 8-15s apart within a burst, no growth). Every large gap
  (87s, 214s, 165s, 221s) sits in the idle time *between* two polls,
  not within a running burst. In other words: this run's QStash chain
  appears to need a resume call to advance past certain points (matching
  the "stalled" reports seen during manual polling), and the session's
  own poll cadence directly shaped the very log pattern being read as
  evidence -- the between-poll silence was being misattributed to
  in-run B.AI retry growth.

  Read correctly, the within-burst evidence is consistent with the fix
  working (flat 8-15s/step, no widening while the chain is actually
  executing) rather than failing. This is not yet a clean positive
  confirmation either, though -- a manually-polled run can't fully rule
  out the original hypothesis, since the polls themselves are now a
  confound in both directions. **To get an unconfounded read: let a
  `bai`-provider audit run to completion or dead-letter via QStash's
  own re-chain alone, with zero manual `resume_run_id` polls in
  between, then pull `/api/agent-worker` logs for that run's full
  timeframe in one shot.** Only then does gap width in the logs mean
  what it was assumed to mean in the original diagnosis.
