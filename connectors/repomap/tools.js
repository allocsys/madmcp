...
export function register(server) {
  ...
  server.tool(
    ...
    "RULE: do NOT re-issue the exact same query/params back-to-back expecting a different answer -- results are deterministic for whatever is currently indexed. A thin or empty result means either the repo isn't scanned yet, the rescan triggered by a recent HEAD change hasn't finished (see CAVEAT above -- wait, don't hammer), or the thing you're looking for genuinely isn't in the graph/embeddings. If you need to retry, change the query wording, check repo_map_scan's jobId status, or space retries out (e.g. one retry after a short wait) instead of calling this tool repeatedly with identical arguments.",
    {
      owner:     z.string().optional().describe(`Repository owner. Defaults to "${DEFAULT_OWNER}" if omitted.`),
