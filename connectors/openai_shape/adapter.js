// ---------------------------------------------------------------------------
// connectors/openai_shape/adapter.js — pure translation between Gemini's
// `contents`/`candidate` wire shape (the lingua franca agent_delegate.js's
// loop, checkpointing, and stuck-loop/step-budget logic are all built
// around) and the OpenAI-compatible `messages`/`choice` shape used by ANY
...
// need to change; connectors/llm/router.js imports directly from here for
// both the "glm" and "groq" branches, since duplicating this file per
// provider would just recreate the drift risk this extraction exists to
// avoid.
//
...
//
// This is the highest-risk file behind every OpenAI-compatible provider
// (a subtle role/shape mismatch here would silently corrupt checkpointed
// conversations for BOTH glm and groq at once) -- see
// test/openai-shape-adapter.test.js for the round-trip test this was
// written against (moved from test/glm-adapter.test.js's original
// coverage, same reasoning as the code move above).
// ---------------------------------------------------------------------------
...
// never produce that finishReason value for any OpenAI-shaped provider
// (GLM or Groq). Not a bug to patch here: it means their failure message
// on that specific edge case is the generic "stopped without a final
// answer -- finishReason: stop" rather than the Gemini-specific
// diagnostic. Do not invent a fake MALFORMED_FUNCTION_CALL equivalent to
// paper over this.
