...
import {
  DEFAULT_OWNER, FRONTEND_ALLOWED_EXTENSIONS, FRONTEND_MAX_ATTEMPTS, FRONTEND_TOTAL_BUDGET_MS,
  FRONTEND_V2_DEFAULT_STEPS, FRONTEND_DESIGNER_V2_ENABLED,
} from "../../config.js";
...
export function register(server) {
  ...
  // -------------------------------------------------------------------
  // delegate_designer_v2 (issue #61, rollout step 5) -- a SEPARATE tool,
  // registered ONLY when FRONTEND_DESIGNER_V2_ENABLED is "true"
  // (config.js). Deliberately not a mode flag on delegate_designer itself
  // and deliberately not registered at all (rather than registered-but-
  // rejecting) when the flag is off, so it can't be discovered or reached
  // for by a calling model before it's been dark-launched on purpose. v1
  // ("delegate_designer" above) is completely unaffected either way --
  // this is additive, not a replacement, until a later change retires v1
  // once v2 is trusted (see the design doc's rollout step).
  //
  // Thin wrapper around runDesignAgent (agent.js): validation and
  // response-shaping here deliberately mirror connectors/gemini/tools.js's
  // delegate_agent (same resume_run_id / max_steps / show_transcript
  // conventions), since this is architecturally the same kind of
  // step-bounded, checkpointable tool-calling loop -- just write-capable
  // and scoped to frontend files instead of read-only/cross-system.
  // -------------------------------------------------------------------
  if (FRONTEND_DESIGNER_V2_ENABLED) {
    server.tool(
      "delegate_designer_v2",
      "DOES: write-capable tool-calling agent for HTML/CSS/SCSS/JSX/TSX/Vue files -- reads files on demand, edits or creates them, validates syntax, iterates -- instead of delegate_designer's one-shot generate->validate->fix loop. FEATURE-FLAGGED ROLLOUT (issue #61): only registered when FRONTEND_DESIGNER_V2_ENABLED=true; prefer plain `delegate_designer` unless you were specifically told to use v2.\n" +
      ...
      "RESUME: failed/partial run -> response includes resume_run_id -> pass back to continue from last completed step instead of restarting.",
      {
        ...
        resume_run_id:   z.string().optional().describe("A runId returned from a previous failed/partial delegate_designer_v2 call. If its checkpoint is still live (1 hour TTL), continues that run's conversation instead of starting fresh."),
      ...
      async ({ owner = DEFAULT_OWNER, repo, branch, task, max_steps, resume_run_id, show_transcript = false }) => {
        ...
        try {
          ...
          return { content: [{ type: "text", text: `delegate_designer_v2 failed: ${err?.message ?? String(err)}` }], isError: true };
        }
