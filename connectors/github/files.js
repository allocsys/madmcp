...
export function register(server) {
  ...
  server.tool(
    ...
    "RULE: question is semantic (\"where is X\") or graph-shaped (\"what calls this\") -> map.query instead (auto-refreshes if stale).\n" +
    "RULE: repo is PUBLIC and goal = run/test/lint code (not just read it) -> git clone via bash_tool instead (github.com/codeload.github.com/raw.githubusercontent.com allowlisted; zero context cost; can execute code). PRIVATE repo -> get_repo_clone_token first to mint a credential.\n" +
  ...
  server.tool(
    ...
    "RULE: looking for WHERE something is implemented, not folder contents -> map.query instead.",
    {
      owner: z.string().optional().describe(`Repository owner. Defaults to "${DEFAULT_OWNER}" if omitted.`),
      repo:  z.string().describe("Repository name"),
  ...
  server.tool(
    ...
    "RULE: next step is locating functionality, not viewing repo shape -> map.query instead.\n" +
    ...
    "RULE: repo is PUBLIC and goal = run/test/lint multiple files (not just read them) -> git clone via bash_tool instead (see read_file's description; zero context cost, can execute code). PRIVATE repo -> get_repo_clone_token first to mint a credential.\n" +
