...
test("formatCascadeLogLine returns null when no fallback fields are set", () => {
...
test("formatCascadeLogLine returns null when candidate is null or undefined", () => {
  assert.strictEqual(formatCascadeLogLine(null, { step: 1, fallbackModel: "gemini-2.5-flash" }), null);
  assert.strictEqual(formatCascadeLogLine(undefined, { step: 1, fallbackModel: "gemini-2.5-flash" }), null);
});

test("formatCascadeLogLine returns model-only message when only _fallbackModelUsed is set", () => {
...
test("formatCascadeLogLine falls back to fallbackModel parameter when _fallbackModelUsed is absent", () => {
...
test("formatCascadeLogLine returns message with key index when both _fallbackModelUsed and _fallbackKeyIndex are set", () => {
